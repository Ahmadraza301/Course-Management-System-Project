...
class courseclass:
    def __init__(self, root):
        ...
        txt_Charges=Entry(self.root, textvariable=self.var_charges, font=("goudy old style", 15, "bold"), bg='white').place(x=150, y=140, width=500)
    ...
    def delete(self):
        ...
        try:
            if self.var_course.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Plaese Slelect the course from List ", parent=self.root)
                else:
